...
function validateEnv(requiredKeys = []) {
  ...
  info({ count: requiredKeys.length }, "✅ Environment variables validated successfully");
}
...
info(systemInfo, "🩺 Startup Health Check");

// ⚠️ Resource warnings
const free = parseFloat(systemInfo.freeMemGB);
const cpus = systemInfo.cpus;
if (free < 0.5) warn({ freeMemGB: systemInfo.freeMemGB }, "Low free memory (<0.5 GB)");
if (cpus < 2) warn({ cpus }, "Low CPU core count (<2 cores)");

info({}, "🚀 Beginning environment validation...");
...
// 🌍 Done
// ------------------------------------------------------------------
info({}, "🌍 All environment variables validated successfully.");
