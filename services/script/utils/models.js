...
import { runModel } from "../../../shared/utils/ai-service.js";
...
export async function generateIntro(sessionId, tone) {
  ...
  return await runModel(prompt, { sessionId, section: "intro" });
}
...
export async function generateMain(sessionId, tone) {
  ...
  return await runModel(prompt, { sessionId, section: "main" });
}

export async function generateOutro(sessionId, tone) {
  const prompt = buildPrompt("outro", { tone, sessionId });
  return await runModel(prompt, { sessionId, section: "outro" });
}